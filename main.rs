...
fn main() {
    ...
    Create a vector to hold all spawned children */
    let mut children = vec![];

    /* For each partition in xs spawn a thread to run map_data concurrently, store
    the threads in the children vecture */
    for i in 0..xs.len() {
        let xs_clone = xs.clone();
        children.push(thread:: spawn(move || map_data(&xs_clone[i])));
    }

    /* Iterate each thread in the children vecture, close the thread when
    it is complete and push the result into intermediate_sums */
    ...
    for i in 0..xs2.len() {
        ...
        children2.push(thread:: spawn(move || map_data(&xs2_clone[i])));
    }
    ...
    for child in children2 {
        intermediate_sums2.push(child.join().unwrap());
    }
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    // Get partition size floor
    let partition_size = v.len() / num_partitions;

    // Get parition size remainder
    let mut remainder = v.len() % num_partitions;

    // Create a vector that will contain vectors of integers
    let mut xs2: Vec<Vec<usize>> = Vec::new();

    let mut i = 0;
    for _ in 0..num_partitions{
        let mut temp_v : Vec<usize> = Vec::new();

        /* Push proper amount of integers into each temp vector. While 
        remainder is > 0, each vector gets partition_size + 1 ints */
        if remainder > 0 {
            for _ in 0..partition_size + 1{
                temp_v.push(v[i]);
                i = i + 1;
            }
        ...
        // Add temp_v to the vector that will be returned by this function
        xs2.push(temp_v);
    }
